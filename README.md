...
- Rider and driver Batch 2 screens live under `app/rider` and `app/driver` and are connected from the existing flow:
  - Rider: `splash -> role-selection -> phone-auth -> otp-verify -> rider -> destination-search -> ride-selection -> matching -> driver-found -> rider/active-trip -> rider/trip-rating -> rider/wallet`
